...
- `create_prompt_template` (zikaria.zikaria.config_utils.PromptTemplateId | None, default: None): The base prompt sent to the Generative AI when completing fields in an existing note. This is formatted dynamically.
- `complete_prompt_template` (zikaria.zikaria.config_utils.PromptTemplateId | None, default: None): Template for note completion prompt
- `image_prompt_template` (zikaria.zikaria.config_utils.PromptTemplateId | None, default: None): Template for image generation
...
- `image_model_name` (str, default: gemini-2.5-flash-image): Model name for image generation
...
- `custom_config` (list): Per-deck and/or per-note-type configuration overrides. Allows setting prompt-related options for specific decks, note types, or combinations. Each entry is a tuple: (NotetypeId | None, DeckId | None, CustomConfig). Overridable keys: create_prompt_template, complete_prompt_template, image_prompt_template, model_temperature, model_name, image_model_name, max_output_tokens.