...
from .config_utils import ConfigType, config, get_effective_config, logger
from .prompts import (
    PromptMode,
...
class ZikariaPrompts:
    ...
    def _finalize_create_note(self, create_note: Note):
        create_note.add_tag(config.get("processed_tag", "zikaria_processed"))
        mw.col.update_note(note=create_note)

        if config.get("suspend_processed_prompt_notes", True):  # Default true
