...
class CustomFormatter(string.Formatter):
    extra_keys = {"prompt", "json_note"}

    def __init__(self, note_type, default_example_count=10, dummy_keys=None):
        """
        :param examples_data: List of example strings
        :param schema_data: A JSON-serializable structure
        """
        self.note_type = note_type
        self.default_example_count = default_example_count
        self.dummy_keys = set() if dummy_keys is None else dummy_keys
    ...
    def format_field(self, value, format_spec):
        ...
        if value is self.examples:
            num = self.default_example_count  # default number of examples
            if format_spec.isdigit():
                num = int(format_spec)
            return example_notes(self.note_type, num)

        if value == self.schema:
            style = format_spec.strip().lower() if format_spec else "readable"
            schema = generate_schema_class(self.note_type)
            if style == "inline":
...
def generate_schema_class(note_type: NoteType) -> dict:
        ...
        if desc := field.get("description"):
            ...
            if "(category)" in desc:  # Special handling for enum generation
                ...
                for nid in note_ids:
                    ...
                    if note and (
                        value := note[name]
                    ):  # Ensure note exists and field has value
    ...
    logger.debug(
        "Generated schema for note type '%s': %s",
        note_type.get("name", "Unknown"),
        schema,
    )
    return schema
...
def example_notes(note_type: NoteType, n: int = 10) -> str:
    ...
    if not all_nids_of_type:
        logger.info(
            f"No notes found for notetype '{note_type.get('name', 'Unknown')}' to generate examples."
        )
        return json.dumps([])
    ...
    try:
        ...
        ValueError
    ) as e:  # Handles issues like sample_size > population_size (should be caught by min)
        logger.error(
            f"Error sampling notes for examples (notetype: {note_type.get('name', 'Unknown')}): {e}",
            exc_info=True,
        )
...
def get_prompt(note, base_prompt: str, current_config: dict, dummy_keys=None) -> str:
    """
    ...
    formatter = CustomFormatter(
        note_type=note_type,
        default_example_count=example_count,
    ...
    try:
        ...
        return formatter.format(base_prompt, **format_args)
    except KeyError as e:
        logger.error(
            f"Missing key in prompt format: {e}. Base prompt: '{base_prompt}', Available Args: {list(format_args.keys())}",
            exc_info=True,
        )
        return ""  # Return empty or raise, depending on desired error handling


def get_create_prompt(note, current_config: dict, **kwargs) -> str:
    """
    ...
    default_create_prompt_template = current_config.get(
        "create_prompt", ""
    ...
    if custom_prompt_tag_val and custom_prompt_tag_val in note.tags:
        base_prompt_template_to_use = (
            field_content  # Note content itself is the template
        )
        logger.debug(
            f"Using custom prompt from note field for note {note.id} due to tag '{custom_prompt_tag_val}'."
        )
        ...
        logger.debug(
            f"Using inline @prompt override from note field for note {note.id}."
        )

    if not base_prompt_template_to_use:
        logger.warning(
            f"Create prompt template is empty for note {note.id}. Effective config: {current_config}"
        )
...
def get_complete_prompt(note, current_config: dict, **kwargs) -> str:
    ...
    if not base_prompt_template_to_use:
        logger.warning(
            f"Complete prompt template is empty for note {note.id}. Effective config: {current_config}"
        )
