...
from .config_utils import logger, config, update_config, ADDON_NAME
from .anki_utils import (
    ensure_tags_exist, 
    API_KEY_MISSING, 
    DISABLE_ON_SYNC,
)
from .core import ZikariaPrompts
from .ui import (
...
def zikaria_process_notes_manual_action(): # Renamed for clarity as an action handler
    ...
    logger.debug(f"User manually triggered Zikaria note processing for addon: {ADDON_NAME}.")
    try:
        ...
        from aqt.utils import showCritical # Keep UI import local to UI-related function
        showCritical(f"An unexpected error occurred during Zikaria processing: {e}", parent=mw)
...
def on_main_window_did_init_hook():
    ...
    """
    logger.debug(f"Zikaria ({ADDON_NAME}): Main window initialized. Ensuring tags exist.")
...
def on_sync_did_finish_hook():
    ...
    """
    logger.debug(f"Zikaria ({ADDON_NAME}): Sync finished. Checking run_on_sync configuration.")
    
    # Access config via the imported config object
    if not config.get("run_on_sync", False):
        logger.info(f"Zikaria ({ADDON_NAME}): Skipping note processing on sync (run_on_sync is false).")
        return

    # Globals are imported from anki_utils
    if DISABLE_ON_SYNC: 
        logger.info(f"Zikaria ({ADDON_NAME}): Note processing on sync is disabled due to a previous error.")
        return

    if API_KEY_MISSING: 
        logger.info(f"Zikaria ({ADDON_NAME}): Note processing on sync is disabled due to missing API key.")
        return
    
    logger.info(f"Zikaria ({ADDON_NAME}): Starting automatic note processing after sync.")
    try:
        processor = ZikariaPrompts(mw=mw, manual_execution=False) # Automatic execution
        processor.process_notes()
        logger.info(f"Zikaria ({ADDON_NAME}): Automatic note processing finished successfully.")
    except Exception e:
        logger.error(f"Error during automatic Zikaria processing after sync: {e}", exc_info=True)
...
qconnect(zikaria_manual_process_action_menu.triggered, zikaria_process_notes_manual_action)
...
if mw.addonManager:
    ...
    zikaria_config_menu_action = QAction(f"Zikaria Addon Configuration ({ADDON_NAME})...", mw)
    qconnect(zikaria_config_menu_action.triggered, lambda: show_config_dialog_action(mw))
...
qconnect(zikaria_process_json_menu_action.triggered, lambda: on_process_json_triggered_action(mw))
...
zikaria_create_from_text_menu_action = QAction("Zikaria: Create Cards from Prompt Text...", mw)
qconnect(zikaria_create_from_text_menu_action.triggered, lambda: open_prompt_text_dialog_action(mw))
mw.form.menuTools.addAction(zikaria_create_from_text_menu_action)


logger.info(f"Zikaria Addon ({ADDON_NAME if ADDON_NAME else __name__}) loaded successfully with modular structure.")
